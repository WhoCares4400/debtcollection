	...
	*/
	...
	Timespace = {
				...
				this.data.events.forEach((v, i) => {
					...
					if (index >= 0) {
						...
						eventElemWidth = (() => {
							...
							eventOverhang = (tableLength < eventOffset + endWidth);

							result = (end) ? (endWidth - extra) : eventElemSpanWidth;
						...
						rowData.eventElem.width(eventElemWidth)
							...
							.attr('title', title.replace(/(<([^>]+)>)/gi, "")+' '+rowData.eventElem.data('time'));
		...
		setDOMEvents: function () {
			...
			this.timeEvents.each(function () {
				...
				if (!elem.data('noDetails')) {
					elem.on('mouseup touchend sim_mouseup', (e, preventScroll) => {
						// Allow if event is not selected and time table has not shifted too much
						if (
							!elem.hasClass(classes.eventSelected)
							&&
							(
								(
						...
						} else if (elem.hasClass(classes.eventSelected)) {
							elem.removeClass(classes.eventSelected);
							ts.display.hide();
						}
		...
		displayEvent: function (elem, preventScroll) {
			...
			//Done event
			let eventIdElem = elem.find('[data-reminder-id]');
			if (eventIdElem.length && eventIdElem.attr('data-done') === "0") {
				let reminderId = eventIdElem.attr('data-reminder-id');
				if (!this.displayTitle.prev().hasClass('complete-reminder-btn')) {
					this.displayTitle.before('<button type="button" class="btn btn-outline-success float-end complete-reminder-btn" data-reminder="' + reminderId + '">Wykonano</button>');
					this.displayTitle.next().append(
						'<div class="row m-0 ms-4">' +
						'<div class="col"><input type="time" class="form-control form-control-sm update-reminder-from-input"></div>' +
						'<div class="col"><input type="time" class="form-control form-control-sm mt-1 update-reminder-to-input"></div>' +
						'</div>' +
						'<div class="row m-0">' +
						'<div class="col"><input type="date" class="form-control form-control-sm update-reminder-date-input"></div>' +
						'<div class="col"><button type="button" class="btn btn-primary btn-sm mt-1 float-end update-reminder-btn"  data-reminder="' + reminderId + '">Aktualizuj</button></div>' +
						'</div>'
					)
					if (window.hasOwnProperty('reminderData')) {
						var rData = getObjectFromArrayByAttributeValue(window.reminderData, 'id', reminderId);

						let tmp = {};
						let startDateTime = (rData.start_time !== null) ? rData.start_time.split(' ') : [null, null];
						let startTime = startDateTime[1];
						let endDateTime = (rData.end_time !== null) ? rData.end_time.split(' ') : [null, null];
						let endTime = endDateTime[1];

						//start end
						let start = (startTime === '00:00:00' || startTime === null) ? 8 : (parseInt(startTime.substring(0, 2)) /* hour */ + (parseInt(startTime.substring(3, 5)) / 60) /* miinute */);
						let end = (endTime === '00:00:00' || endTime === null) ? 20 : (parseInt(endTime.substring(0, 2)) /* hour */ + (parseInt(endTime.substring(3, 5)) / 60) /* miinute */);

						tmp.start = start;
						tmp.end = (end > start) ? end : Math.min( (start + 1), 24 );

						let reminderStartTime = timespaceGetHours(tmp.start, true)+':'+timespaceGetMinutes(tmp.start);
						let reminderEndTime = timespaceGetHours(tmp.end, true)+':'+timespaceGetMinutes(tmp.end);

						$('.update-reminder-date-input').val( rData.start_time.substring(0,10) );
						$('.update-reminder-from-input').val( reminderStartTime );
						$('.update-reminder-to-input').val( reminderEndTime );
					}
				} else {
					this.displayTitle.prev().show().attr('data-reminder', reminderId)
				}
			} else if ( this.displayTitle.prev().hasClass('complete-reminder-btn') ) {
				this.displayTitle.prev().remove();
			}

			this.displayBody.empty()
				.append(elem.data('description'));
			elem.addClass(classes.eventSelected);
			$('#reminder-table tbody tr.outline-primary').removeClass('outline-primary');
